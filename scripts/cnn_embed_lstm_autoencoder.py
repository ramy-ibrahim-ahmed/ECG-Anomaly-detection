...
cnn_extractor.compile(optimizer="adam", loss="mae")
...
cnn_embed_lstm_autoencoder = Sequential(
    [
        # Encoder
...
cnn_embed_lstm_autoencoder.compile(
    optimizer="adam",
    loss="mae",
)
cnn_embed_lstm_autoencoder.summary()

# NET TRAINING
history = cnn_embed_lstm_autoencoder.fit(
    cnn_features_train,
    cnn_features_train,
    epochs=50,
    batch_size=32,
...
reconstructions = cnn_embed_lstm_autoencoder.predict(cnn_features_test)
reconstruction_error = np.mean(
    np.abs(reconstructions - cnn_features_test), axis=2
).mean(axis=1)
...
plt.legend()
plt.savefig(
    r"D:\ramy\Omnetrex\Heartbeat anomaly detection on time series\images\evaluation\CNNEmbedINGLSTMAutoencoder.png"
)
plt.show()
...
plt.savefig(
    r"D:\ramy\Omnetrex\Heartbeat anomaly detection on time series\images\evaluation\CNNEmbedINGLSTMAutoencodercm.png"
)
plt.show()

# SAVE NET
cnn_embed_lstm_autoencoder.save(
    r"D:\ramy\Omnetrex\Heartbeat anomaly detection on time series\Models\CNNEmbedINGLSTMAutoencoder.keras"
)

cnn_extractor.save(
    r"D:\ramy\Omnetrex\Heartbeat anomaly detection on time series\Models\CNNEmbed.keras"
)

utils.plot_model(
    cnn_embed_lstm_autoencoder,
    to_file=r"D:\ramy\Omnetrex\Heartbeat anomaly detection on time series\images\architecture\cnn_embed_lstm_autoencoder.png",
    show_shapes=True,
...
utils.plot_model(
    cnn_extractor,
    to_file=r"D:\ramy\Omnetrex\Heartbeat anomaly detection on time series\images\architecture\cnn_embed.png",
    show_shapes=True,
    show_layer_names=True,
)
